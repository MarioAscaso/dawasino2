package com.example.dawCasino2Back.blackjack.application.services;
...
import com.example.dawCasino2Back.blackjack.infrastructure.GameDTO;
import com.example.dawCasino2Back.blackjack.domain.BlackjackGame;
import com.example.dawCasino2Back.blackjack.domain.repositories.BlackjackGameRepository;
...
public class BlackjackService {
    ...
    // --- MAPEO DTO: LÓGICA DE OCULTACIÓN ACTUALIZADA ---
    private GameDTO mapToDTO(GameSession session, double balance, String status) {
        ...
        if (status.equals("PLAYING") && visibleDealerCards.size() >= 2) {
            ...
            // 1. Ocultamos la PRIMERA carta (índice 0)
            visibleDealerCards.set(0, "HIDDEN");

            // 2. Si hubiera cartas extra por algún motivo extraño, también ocultas
            // (Aunque en turno de jugador el dealer solo tiene 2)

            // 3. Calculamos puntuación SOLO de las cartas visibles (desde índice 1)
    ...
    private static class GameSession {
        ...
        // NUEVO ORDEN DE REPARTO: P1 -> D1(Hole) -> P2 -> D2(Visible)
